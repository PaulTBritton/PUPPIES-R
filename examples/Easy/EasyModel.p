setattr(name="Easy Model",iterations=700,seed=1)
#print("easymodel.p")
#print(ls(parent.frame()))
E5 <- unifD(4,6)
#print(E5)
E3 <- 3
A3 <- triaD(2,3,4)
B5 <- betaD(alpha=A3,beta=6)
B4 <- empeD("data/B4.dat")
#B3 <- lognD(mu=log(7.00E-05)-(1e-4^2)/2,sigma=1e-4)
B3 <- lognD(mu=-9.57,sigma=1e-4)
#print(B3)
B2 <- lognD(mean=6.00E-04,EF=E5)
#print(B2)
B1 <- lognD(median=B2,EF=E5)
#print(B1)
S5 <- B1&B2&B3
S4 <- B3&(NOT(B5)|(B2&B1))
S3 <- AND("B[1-3]")
S2 <- OR("B[1-3]")
S1 <- MN(3,10,S2)
M2 <- NOT(cos(S3))+sin(S1)+S2
M1 <- S1*5.00E-01
testM <- rbeta(500,A3,6)
pplot(plotname="Easyplot1.tiff",lst=plotlist(B4,B5,testM,M2,M1))
elist <- plotlist(B4,B5,M2,M1)
pplot(plotname="Easyplot2.tiff",lst=elist)
eventstoplot <- plotlist("B4 name"=B4,B5,"M2 descr"=M2,M1)
pplot(plotname="Easyplot3.tiff",lst=eventstoplot) #,filter="M?|S?")
#print(ls())
#print(list(S2,S1,M2,M1))
#print(alist(S2,S1,M2,M1))
print("Easy Model Complete")
